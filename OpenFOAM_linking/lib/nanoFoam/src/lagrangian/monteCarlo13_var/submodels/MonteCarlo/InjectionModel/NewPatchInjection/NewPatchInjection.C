...
Foam::NewPatchInjection<CloudType>::NewPatchInjection
(
    ...
    fraction_(1.0)
{
    if (patchId_ < 0)
    {
        FatalErrorIn
        (
            "NewPatchInjection<CloudType>::NewPatchInjection"
            "("
                "const dictionary&, "
                "CloudType&"
            ")"
...
Foam::NewPatchInjection<CloudType>::NewPatchInjection
(
    const NewPatchInjection<CloudType>& im
)
:
...
template<class CloudType>
void Foam::NewPatchInjection<CloudType>::updateMesh()
{
...
template<class CloudType>
Foam::scalar Foam::NewPatchInjection<CloudType>::timeEnd() const
{
...
template<class CloudType>
Foam::label Foam::NewPatchInjection<CloudType>::parcelsToInject
(
    const scalar time0,
    const scalar time1
)
...
template<class CloudType>
Foam::scalar Foam::NewPatchInjection<CloudType>::volumeToInject
(
    const scalar time0,
    const scalar time1
)
...
template<class CloudType>
void Foam::NewPatchInjection<CloudType>::setPositionAndCell
(
    const label,
    const label,
    const scalar,
...
)
{
    if (cellOwners_.size() > 0)
    {
        ...
        const polyPatch& patch = this->owner().mesh().boundaryMesh()[patchId_];
	face patchFace = patch[20];
	//labelList patchPoints = patchFace;
	const pointField& points = this->owner().mesh().points();

	const vector boundary = this->owner().mesh().Sf().boundaryField()[patchId_][20];
	const scalar faceMag = this->owner().mesh().magSf().boundaryField()[patchId_][20];
	const vector faceCenter = this->owner().mesh().Cf().boundaryField()[patchId_][20];
	Info << boundary/faceMag << " with center: " << faceCenter << endl;
	forAll(patchFace, faceI)
	{
	   label patchFaceI = patchFace[faceI];
	   Info << "Coordinates: " << points[patchFaceI] << " of Vertex" << patchFaceI << endl;
	}
	...
	//Info << "tetFaceI faceCentre: " << this->owner().mesh().faceCentres()[tetFaceI] << endl;
        tetPtI = 1;

	const labelList& cFaces = this->owner().mesh().cells()[cellOwner];
        const vector& pc = this->owner().mesh().C()[cellOwner];

	forAll(cFaces, cFaceI)
	{
	    label faceI = cFaces[cFaceI];
	    const vector& faceVector = this->owner().mesh().faceCentres()[faceI];
	    vector vTmp = faceVector - pc;
	    scalar tmp = mag(vTmp);
	    Info << "Face vector component " << cFaceI << " with vector: " << vTmp << endl;
	}
...
template<class CloudType>
void Foam::NewPatchInjection<CloudType>::setProperties
(
    const label,
    const label,
    const scalar,
...
template<class CloudType>
bool Foam::NewPatchInjection<CloudType>::fullyDescribed() const
{
    return false;
}
...
template<class CloudType>
bool Foam::NewPatchInjection<CloudType>::validInjection(const label)
{
    return true;
}
